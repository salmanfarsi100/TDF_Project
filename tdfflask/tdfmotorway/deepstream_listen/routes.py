...
deepstreamlistenbp = Blueprint("deepstreamlistenbp", __name__, static_folder='static', template_folder='templates')
...
@deepstreamlistenbp.route("/<key_time>", methods=['GET', 'POST'])
def listen_frame(key_time):
	yymmdd = str(date.today())
	date_today = yymmdd[8:10] + '/' + yymmdd[5:7] + '/' + yymmdd[0:4] 
	key_datetime = str(date_today) + ' ' + str(key_time)
	print(key_datetime)
	result = Camera.query.filter(Camera.datetime >= key_datetime)
	d = 'no entries found'
	if result:
		new_d = []
		for i in result:
